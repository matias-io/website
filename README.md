...
Generated documents identify Matias and expand related project IDs into their existing titles and summaries. Retrieval searches the current question, filters by language and checks source hashes before passing evidence and conversation context to Workers AI. The guide uses Llama 3.1 8B Instruct Fast with structured claims and source IDs; the Worker validates each claim's IDs and renders citations. Model and retrieval settings live in `src/worker/retrieval.ts`. Operational logs record retrieval counts and failure stages, never questions, answers or verification tokens.
