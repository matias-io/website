...
Generated documents identify Matias and expand related project IDs into their existing titles and summaries. Retrieval filters by language and checks source hashes before passing evidence to Workers AI. Cloudflare searches the first question directly; native follow-up rewriting receives bounded conversation history explicitly to resolve references while preserving independent questions. The guide uses Llama 3.3 70B Instruct FP8 Fast with structured claims and source IDs; the Worker validates each claim's IDs and renders citations. Model and retrieval settings live in `src/worker/retrieval.ts`. Operational logs record retrieval counts and failure stages, never questions, answers or verification tokens.

The build writes ignored Markdown to `.local/knowledge/` and a trusted citation manifest to `src/worker/generated/manifest.json`. Sync has two bounded phases: it submits uploads, then polls indexing against a shared 15-minute deadline, with at most four concurrent HTTP requests in each phase. It verifies retrieval while retaining previous documents and retries transient failures with API backoff. After successful Worker deployment, `knowledge:prune` removes stale files owned by this project. A failed deployment leaves the previous generation's evidence available. Content hashes prevent an older source from supporting a changed page. Keep the generated manifest with the release.
...
Provision AI Gateway `matiass-assistant`, AI Search `matiass-public` in the `default` namespace and a Turnstile widget. Instance creation via the sync script connects that gateway and configures multilingual BGE-M3 hybrid retrieval and required metadata. Sync verifies the gateway assignment; set `AI_GATEWAY_ID` when using a different project gateway. Keep gateway payload logging and caching off, and configure a blocking budget before enabling chat. Disable AI Search public endpoints, including namespace access to this instance. Add the actual workers.dev hostname to preview `ALLOWED_HOSTNAMES` and the widget's allowed domains. The contact form stays disabled on preview; LinkedIn remains available. Enable the form after matiass.ca transfers to this account and email routing/sending is configured.
